...
console.log(`  Partidas continentais: ${Object.values(world.continentalMatches).reduce((s, m) => s + m.matches.length, 0)}`);  check('5 países (inclui Brasil)', world.countries.length === 5);
  check('≥ 336 clubes (4×3×20 + Brasil A/B/C/D 20×3+96)', Object.keys(world.clubs).length >= 336);
...
check('copa com ≥ 59 partidas (exceto playoff de acesso)', Object.values(world.cupMatches).filter((s) => s.matches.length > 0).every((s) => s.matches.length >= 4));
...
console.log('💬 Testando conversas com jogadores...');
{
  ...
  if (p) {
    ...
    const moraleBefore = p.morale;
    const happinessBefore = p.happiness;
    ...
    check('moral/satisfação alterada pela resposta', p.morale !== moraleBefore || p.happiness !== happinessBefore, `(moral ${moraleBefore} → ${p.morale}, sat ${happinessBefore} → ${p.happiness})`);
